...
// @version      1.0.0
// @author       gjy
// @description  为FTP文件云网页特定标注影响因子的插件
...
// @match        http://xx.xx.xx.xx:xx/*
// @icon         https://img1.baidu.com/it/u=4253150609,2988920661&fm=253&fmt=auto&app=138&f=PNG?w=500&h=500
// ==/UserScript==
( () => {
    'use strict';
    var inputs=[]
        ...
        crossref(doi){
            ...
            $.getJSON(url, (res) => {
                if( res ){
                    ...
                    try { if( !pdata.author ){ var au = res.z_authors, author = ''; for (var i = 0; i < au.length; i++) { author += au[i].given + ' ' + au[i].family; if(i < au.length - 1 ) author += '; ' }
                          if( author.length > 100 ) { author = author.slice(0,99); author = author.slice(0,author.lastIndexOf(";")); author += ', et al.' } pdata.author = author; } } catch(e) {}
            ...
            $.getJSON(crourl, (res) => {
                if( res.message ){
                    ...
                    try { var au = m.author, author = ''; for (var i = 0; i < au.length; i++) { author += au[i].given + ' ' + au[i].family; if(i < au.length - 1 ) author += '; ' }
                          if( author.length > 100 ) { author = author.slice(0,99); author = author.slice(0,author.lastIndexOf(";")); author += ', et al.' } pdata.author = author; } catch(e) {}
    ...
    let utils = {
        ...
        getRankSpan( node, jTitle, inner ) {
            if( !$(node).attr("ranked") ||true){
                var go=$(node).find(".rankdiv")
                if(go.length>0){
                    go[0].remove()
                }
                ...
                if( rdata.length > 1 ){
                    if(inner){ $(node).append( rankDiv ); } else { $(node).after( rankDiv ); }
                }
                else{
                    var rankSpan = document.createElement('span');
                    $(rankSpan).addClass("rankInfo");
                    $(rankSpan).css("background","#a5a5a5").text("无信息").attr("title", "");
                    $( rankDiv ).append( rankSpan )
                    $(node).append( rankDiv );
                }
            }
    ...
    let start = {
        ...
        addRankIco() {
            ...
            if( base.t(/iop\.org/) ){
                ...
                $("h3.t a").each((i,e) => {
                    jt = $(e).parent().parent().find(".sc_info > span:nth-child(2) > a").attr("title");
                    if(jt) utils.getRankInfo( e,jt,true )
                })
            }else if ( base.t(/10\.3\.11\.178:8080/) ){
                inputs=document.getElementsByName("title")
                var jts=[]
                for(var i=0;i<inputs.length;i++){
                    var title=inputs[i].value
                    var e=inputs[i]
                    var idx=title.indexOf('.')
                    title=title.substring(4,idx)
                    var url=encodeURI("https://api.oadoi.org/v2/search?email=unpaywall@impactstory.org&query= "+title)
                    var sum=0
                    $.ajax({
                        //几个参数需要注意一下
                        type: "get", //方法类型
                        ...
                        success: function(result) {
                            if(result.results[0]){
                                var jt=result.results[0].response.journal_name
                                utils.getRankSpan($(inputs[sum]).parent(),jt,true)
                            }else{
                                utils.getRankSpan($(inputs[sum]).parent(),"no",true)
                            }
                            sum++
                        },
                        error: function() {
    ...
    let main = {
        init() {
            var firstname=''
            if( !$.isEmptyObject( tdata )||true){
                setInterval(() => {
                    start.initSciCRX();
                    if(firstname!=document.getElementsByName("title")[0].value){
                        
                        start.addRankIco();
                        firstname=document.getElementsByName("title")[0].value
                    }
                }, 1e3)
            }
